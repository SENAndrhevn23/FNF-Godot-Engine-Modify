...
# ======= OPTIMIZATION STATE =======
var note_pool: Array = []
var recycle_queue: Array = [] # used if disableGC = true and we can't free immediately
...
func _ready():
    ...
    # Initialize note pool if enabled
    init_note_pool()
...
func player_input():
    # If player strum not available, or botplay is enabled for player, skip manual input
    if (PlayerStrum == null or Settings.botPlay):
        return
...
func button_logic(line, note):
    ...
    if (Input.is_action_just_pressed(action)):
        ...
        if (curNote != null):
            ...
            # Call the note's hit logic
            curNote.note_hit(distance)
            # Recycle (defer to let note handle any internal effects)
            call_deferred("recycle_note", curNote)
            ...
            for dupedNote in activeNotes:
                ...
                if (dupedNote.note_type == curNote.note_type):
                    if (dupedNote.strum_time <= curNote.strum_time + 0.01):
                        # recycle duplicates instead of queue_free
                        recycle_note(dupedNote)
...
func get_section():
    if (sections == null or sections.empty()):
        return
...
func spawn_note(dir, strum_time, sustain_length):
    ...
    if (strumLine != null):
        # use pooled note if available / allowed
        var note = get_note()
        ...
        # add the note to the scene under the strum's Notes node
        strumLine.get_node("Notes").add_child(note)
        note.visible = true
        
        # FAST BOTPLAY: if botplay active and fastBotplay enabled, instantly hit the note to avoid heavy logic
        if Settings.botPlay and Settings.fastBotplay and note.must_hit:
            # perfect timing (0ms distance)
            note.note_hit(0)
            call_deferred("recycle_note", note)
...
func setup_characters():
    ...
    if (PlayerCharacter != null):
        ...
        if (PlayerCharacter.girlfriendPosition or EnemyCharacter.girlfriendPosition):
...
# =========================
# NOTE POOL / RECYCLE API
# =========================

func init_note_pool():
    # Preallocate notes if note pooling enabled
    if (not Settings.notePooling):
        return
    var pool_size = 0
    # default fallback if setting missing
    if (Settings.has_method("notePoolSize") == false and typeof(Settings.notePoolSize) == TYPE_NIL):
        pool_size = 10000
    else:
        pool_size = int(Settings.notePoolSize)
    if pool_size <= 0:
        return
    # Preload NOTE_NODE instances but don't add to scene tree yet
    for i in range(pool_size):
        var n = NOTE_NODE.instance()
        n.visible = false
        # any reset init here if your Note scene supports it
        note_pool.append(n)

func get_note():
    # returns a ready-to-use note node (pooled or new)
    if (Settings.notePooling and note_pool.size() > 0):
        var n = note_pool.pop_back()
        # ensure it's reset
        reset_note(n)
        return n
    # fallback: dynamic instantiation
    var new_n = NOTE_NODE.instance()
    reset_note(new_n)
    return new_n

func recycle_note(note):
    # safely recycle or free a note
    if not is_instance_valid(note):
        return
    # try to remove from parent if present
    if note.get_parent() != null:
        note.get_parent().remove_child(note)
    reset_note(note)
    note.visible = false
    if (Settings.noteRecycling):
        note_pool.append(note)
    else:
        # If GC disabling requested, push to recycle queue to free later (avoid heavy queue_free during gameplay)
        if (Settings.disableGC):
            recycle_queue.append(note)
        else:
            # normal free
            note.queue_free()

func reset_note(note):
    # Try to reset note properties to a safe default
    # If your Note scene has its own reset() method, prefer that
    if note.has_method("reset"):
        note.reset()
        return
    # Best-effort reset:
    note.visible = false
    note.position = Vector2(0, 1280)
    if note.has("strum_lane"):
        note.strum_lane = null
    note.strum_time = 0
    note.sustain_length = 0
    note.note_type = 0
    note.must_hit = false
    # stop any animation or tween if present
    if note.has_node("AnimationPlayer"):
        var ap = note.get_node("AnimationPlayer")
        ap.stop()

# Call this when a song/level ends to actually free queued nodes if needed
func flush_recycle_queue():
    for n in recycle_queue:
...
func restart_playstate():
    # flush queued frees if disableGC used
    flush_recycle_queue()
